...
import { Heart, ShoppingBag, Star, ChevronRight, CheckCircle2, MessageSquarePlus, X } from "lucide-react";
...
useEffect(() => {
  axios
    .get(`http://localhost:3001/api/user/auth/feedback/${id}`)
...
setReviewError("");
  try {
    ...
    await axios.post(
      "http://localhost:3001/api/user/auth/add",
      {
        productId: id,
        rating,
        message,
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3001/api/product/product/${id}`)
...
//   return (
//   <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 px-16 py-10  bg-[#f6f4f1]">
...
//   {/* RIGHT CONTENT */}
//   <div className="px-12 pt-20 max-w-xl bg-[#f6f4f1]">
...
//   <div className="mt-10 mb-10 border p-6 bg-white">
...
//     reviews.map((r) => (
//       <div key={r._id} className="border-t py-4">
...
//     <button  onClick={(e)=>{handleAddToCart(e,product._id)}} className=" bg-black text-white  tracking-widest text-sm mb-4">
...
//       {/* <img className="w-6 h-6 " src={whish} alt="" /> */}
//       <button className=" gap-2 border flex justify-center items-center p-2 text-black  tracking-widest text-sm mb-4" >
//         <img
//         onClick={(e) => handleWishlistClick(e, product._id)}
...
//   <p>
//     {product.description || "Premium quality fabric with modern fit."}
//   </p>
...
//     {/* BUY NOW */}
//     <button onClick={handleBuyNow} className="mt-6 bg-black text-white px-8 py-3">
//       Buy now
//     </button>
